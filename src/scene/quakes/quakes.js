...
export const Quakes = () => {
    const quakes = useSelector(state => state.quakes.quakes)
    ...
    const material = useMemo(
        () => createShaderMaterial({
            minTime: store.getState()?.render?.minDate || 0,
            maxTime: store.getState()?.render?.maxDate || DATE_2100,
        }),
    ...
    useFrame(() => {
      if (meshRef.current) {
        ...
        const minTime = state?.render?.minDate || 0
        const maxTime = state?.render?.maxDate || DATE_2100
